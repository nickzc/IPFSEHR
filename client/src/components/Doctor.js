...
const Doctor = ({ ipfs, mediChain, account }) => {
  ...
  const handleShowRecordModal = async (patient) => {
    var record = {};
    await fetch(
      `${process.env.REACT_APP_INFURA_DEDICATED_GATEWAY}/ipfs/${patient.record}`
    )
      .then((res) => res.json())
  ...
  const submitDiagnosis = async (e) => {
    ...
    ipfs.add(record).then((result, error) => {
      if (error) {
        ...
        mediChain.methods
          .insuranceClaimRequest(patient.account, result.path, charges)
          .send({ from: account })
                            ...
                            <tr key={idx + 1}>
                              ...
                              <td>
                                {treatment.prescription ? (
                                  <Link
                                    to={`${process.env.REACT_APP_INFURA_DEDICATED_GATEWAY}/ipfs/${treatment.prescription}`}
                                    target="_blank"
                                  >
